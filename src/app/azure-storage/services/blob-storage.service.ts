...
import { distinctUntilChanged, scan, startWith, switchMap, map } from "rxjs/operators";
import {
...
export class BlobStorageService {
  ...
  containers$ = this.getStorageOptions().pipe(
    switchMap(options => this.getContainers(options))
  );

  getStorageOptionsWithContainer(containerName): Observable<BlobContainerRequest> {
    return this.getStorageOptions().pipe(
      map((options) => ({ ...options, containerName }))
    );
  }
